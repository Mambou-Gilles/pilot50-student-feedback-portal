document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderProfessionalUI(match) {
        // 1. Filter out null or incomplete scores so we only calculate based on what actually exists
        const availableScores = [];
        if (match.scores.quality && match.scores.quality.score !== "") availableScores.push(parseFloat(match.scores.quality.score) || 0);
        if (match.scores.completeness && match.scores.completeness.score !== "") availableScores.push(parseFloat(match.scores.completeness.score) || 0);
        if (match.scores.engagement && match.scores.engagement.score !== "") availableScores.push(parseFloat(match.scores.engagement.score) || 0);
        if (match.scores.adherence && match.scores.adherence.score !== "") availableScores.push(parseFloat(match.scores.adherence.score) || 0);
        ...
        // 3. Dynamic Grid Class (Automatically shrinks to 2 columns for Cohort 18+ layouts)
        const gridCols = availableScores.length <= 2 ? "sm:grid-cols-2 lg:grid-cols-2 max-w-2xl mx-auto" : "sm:grid-cols-2 lg:grid-cols-4";
            ...
            <div class="animate-popIn space-y-10">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 ${gridCols} gap-6">
                        ${(match.scores.quality && match.scores.quality.score !== "") ? createFlipCard("Response Quality", match.scores.quality, "text-blue-600", "bg-blue-600", "Thoughtful, well-articulated answers that reflect your learning.") : ''}
                            
                        ${(match.scores.completeness && match.scores.completeness.score !== "") ? createFlipCard("Task Completion", match.scores.completeness, "text-indigo-600", "bg-indigo-600", "Answering all required questions and completing assigned tasks.") : ''}
                            
                        ${(match.scores.engagement && match.scores.engagement.score !== "") ? createFlipCard("Engagement", match.scores.engagement, "text-violet-600", "bg-violet-600", "Demonstrating effort, critical thinking, and enthusiasm.") : ''}
                            
                        ${(match.scores.adherence && match.scores.adherence.score !== "") ? createFlipCard("Adherence", match.scores.adherence, "text-sky-600", "bg-sky-600", "Following instructions, including formatting and content requirements.") : ''}
                    </div>
                </div>
...
//     function renderProfessionalUI(match) {
//         // 1. Filter out null scores so we only calculate based on what exists
//         const availableScores = [];
//         if (match.scores.quality) availableScores.push(parseFloat(match.scores.quality.score) || 0);
//         if (match.scores.completeness) availableScores.push(parseFloat(match.scores.completeness.score) || 0);
//         if (match.scores.engagement) availableScores.push(parseFloat(match.scores.engagement.score) || 0);
//         if (match.scores.adherence) availableScores.push(parseFloat(match.scores.adherence.score) || 0);
...
//         // 3. Dynamic Grid Class (Changes from 4 columns to 2 if scores are missing)
//         const gridCols = availableScores.length <= 2 ? "sm:grid-cols-2 lg:grid-cols-2 max-w-2xl mx-auto" : "sm:grid-cols-2 lg:grid-cols-4";
...
//                     <div class="grid grid-cols-1 ${gridCols} gap-6">
//                         ${match.scores.quality ? createFlipCard("Response Quality", match.scores.quality, "text-blue-600", "bg-blue-600", "Thoughtful, well-articulated answers that reflect your learning.") : ''}
                            
//                         ${match.scores.completeness ? createFlipCard("Task Completion", match.scores.completeness, "text-indigo-600", "bg-indigo-600", "Answering all required questions and completing assigned tasks.") : ''}
                            
//                         ${match.scores.engagement ? createFlipCard("Engagement", match.scores.engagement, "text-violet-600", "bg-violet-600", "Demonstrating effort, critical thinking, and enthusiasm.") : ''}
                            
//                         ${match.scores.adherence ? createFlipCard("Adherence", match.scores.adherence, "text-sky-600", "bg-sky-600", "Following instructions, including formatting and content requirements.") : ''}
//                     </div>
//                 </div>
